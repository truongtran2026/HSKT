...
// xem lib/transitPositionMismatches.ts để hiểu đầy đủ gốc rễ).
//
// SỬA 2026-08-02 (cùng ngày, phản hồi người dùng: "chưa chắc là device_position_
// map đúng đâu; đúng ra bạn phải để tôi chọn chứ sao áp đặt như vậy được. thậm
// chí cả 2 đều sai và nhập cho đúng lại nữa kia") — bản đầu tự động coi thư
// viện LUÔN đúng, chỉ cho sửa 1 chiều (ghi đè Chuyển tiếp). Đổi hẳn: hiện 2
// giá trị NGANG HÀNG (không còn nhãn "đúng"/"sai"), người dùng tự chọn 1
// trong 3 hướng — không có lựa chọn mặc định nào được tô sẵn.
export default function TransitPositionMismatchTab({ items }: { items: TransitPositionMismatch[] }) {
  ...
  const filtered = useMemo(() => {
    ...
    return remaining.filter(
      (it) => it.deviceName.toLowerCase().includes(q) || it.rackCode.toLowerCase().includes(q) || it.rawText.toLowerCase().includes(q)
    );
  }, [remaining, search]);
  ...
  if (items.length === 0) {
    return <EmptyState text="Không có dòng &quot;Chuyển tiếp&quot; nào lệch tọa độ ODF so với thư viện Vị trí thiết bị." />;
  }
...
type Choice = "transit" | "library" | "custom";

function MismatchRow({ item, onDone }: { item: TransitPositionMismatch; onDone: () => void }) {
  ...
  return (
    <li className="rounded border border-rose-200 bg-white px-2 py-1.5">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <a
          href={`/odf-trunk/${item.rackId}#port-${item.portId}`}
          target="_blank"
          rel="noopener noreferrer"
          className="text-xs text-rose-500 hover:underline"
          title="Xem port này ở Hồ sơ ODF Trung kế (mở tab mới)"
        >
          {item.rackCode} port {item.portNumber}
        </a>
      ...
      <div className="mt-1.5 flex flex-wrap items-start gap-4 text-xs">
        <label className="flex items-start gap-1.5">
          <input type="radio" name={`choice-${item.transitLinkId}`} className="mt-0.5" checked={choice === "transit"} onChange={() => setChoice("transit")} />
          <span>
            <span className="text-rose-500">Chuyển tiếp: </span>
            <span className="font-mono">{item.transitOdfPart}</span>
            <br />
            <span className="text-slate-400">dùng giá trị này (ghi đè thư viện)</span>
          </span>
        </label>
        <label className="flex items-start gap-1.5">
          <input type="radio" name={`choice-${item.transitLinkId}`} className="mt-0.5" checked={choice === "library"} onChange={() => setChoice("library")} />
          <span>
            <span className="text-rose-500">Vị trí thiết bị: </span>
            <span className="font-mono">{item.libraryOdfPosition}</span>
            <br />
            <span className="text-slate-400">dùng giá trị này (ghi đè Chuyển tiếp)</span>
          </span>
        </label>
        <label className="flex items-start gap-1.5">
          <input type="radio" name={`choice-${item.transitLinkId}`} className="mt-0.5" checked={choice === "custom"} onChange={() => setChoice("custom")} />
          <span>
