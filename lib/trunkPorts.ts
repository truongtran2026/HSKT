...
export function matchTrunkPosition(text: string, trunkPorts: TrunkPortRow[]): TrunkPositionMatch {
  ...
  for (const rackCode of rackCodes) {
    ...
    if (requestedPortNumbers.length === 0) {
      ...
      return { matched: true, rackCode, cableRouteName, requestedPortNumbers: [] };
    }
    ...
    return { matched: true, rackCode, cableRouteName, requestedPortNumbers, resolvedPorts, invalidPortNumbers };
  }
